...
exports.createMarketplaceCommand = createMarketplaceCommand;
const commander_1 = require("commander");
const node_fetch_1 = __importDefault(require("node-fetch"));
function createMarketplaceCommand() {
    const marketplaceCmd = new commander_1.Command('marketplace')
    ...
    marketplaceCmd
        ...
        try {
            ...
            if (options.priceRange)
                queryParams.append('priceRange', options.priceRange);
            if (options.rating)
                queryParams.append('rating', options.rating);
            queryParams.append('sort', options.sort);
            queryParams.append('limit', options.limit);
            const response = await (0, node_fetch_1.default)(`${globalOpts.gateway}/marketplace/listings?${queryParams}`, {
