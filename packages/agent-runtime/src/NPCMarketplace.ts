...
export interface MarketplaceListing {
    id: string;
    npcTemplate: any; // NPCTemplate from npc-builder
    seller: string;
    price: string; // in wei
    tokenAddress: string;
...
export interface MarketplaceStats {
    ...
    trendingNPCs: MarketplaceListing[];
...
export class NPCMarketplace {
    ...
    constructor(apiBaseUrl: string = 'https://api.npc-marketplace.com') {
    ...
    async listNPC(
        ...
        price: string,
        tokenAddress: string,
        category: 'free' | 'premium' | 'exclusive' = 'premium',
        tags: string[] = []
    ): Promise<string> {
        try {
            ...
            console.log(`NPCMarketplace: Listed NPC "${npcTemplate.name}" for ${ethers.formatEther(price)} STT`);
            return listingId;
     ...
     */
    async purchaseNPC(listingId: string): Promise<any> {
        ...
        try {
            ...
            console.log(`NPCMarketplace: Simulating purchase of "${listing.npcTemplate.name}" for ${ethers.formatEther(listing.price)} STT`);
    ...
    async searchNPCs(query: {
        ...
        if (query.searchTerm) {
            ...
            results = results.filter(listing => 
                listing.npcTemplate.name.toLowerCase().includes(term) ||
        ...
        if (query.tags && query.tags.length > 0) {
            results = results.filter(listing => 
                query.tags!.some(tag => listing.tags.includes(tag))
            );
        }
    ...
    async getMarketplaceStats(): Promise<MarketplaceStats> {
        ...
        const topCreators: CreatorProfile[] = [];
